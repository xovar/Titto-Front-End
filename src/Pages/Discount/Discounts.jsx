...
export default function Discounts() {
  ...
  // --- LOGIC: DISCOUNT & FILTERS ---
  let processedProducts = products.filter(product => {
    
    // ⚡ 💥 ০. ডিসকাউন্ট চেক লজিক:
    // ১. প্রোডাক্টের মূল `discount` ফিল্ড থাকলে
    // ২. অথবা regular_price > price হলে
    // ৩. অথবা কোনো variant-এ discount থাকলে
    const hasProductDiscount = Number(product.discount) > 0 || Number(product.regular_price) > Number(product.price);
    const hasVariantDiscount = product.variants && product.variants.some(v => Number(v.discount) > 0 || Number(v.regular_price) > Number(v.price));
    
    const isDiscounted = hasProductDiscount || hasVariantDiscount;
    ...
    // সব শর্তের সাথে isDiscounted ফিল্টার মিলানো হলো
    return isDiscounted && matchesPrice && matchesCategory && matchesSize && matchesColor && matchesBrand;
  });
  ...
  return (
    <div className="max-w-360 mx-auto px-4 md:px-8 py-10 font-sans text-neutral-800">
      ...
      {/* PAGE HEADER BANNER (OPTIONAL BUT RECOMMENDED) */}
      <div className="mb-8 p-6 bg-rose-50 border border-rose-100 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-rose-600 uppercase tracking-tight">Special Offers & Discounts 🏷️</h1>
          <p className="text-xs text-neutral-500 mt-1 font-medium">Grab your favorite items at discounted prices before stock runs out!</p>
        </div>
        <span className="bg-rose-600 text-white text-xs font-bold px-4 py-2 rounded-full uppercase tracking-wider">
          Up to 50% OFF
        </span>
      </div>
          ...
          <div className={`mb-10 w-full ${viewMode === 'grid' && !loading ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6' : 'flex flex-col gap-6'}`}>
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 w-full col-span-full bg-white border border-neutral-200 rounded-xl shadow-sm">
                ...
                <p className="text-sm text-neutral-400 font-bold uppercase mt-4 tracking-wider">Loading offers...</p>
              </div>
              ...
              <p className="text-neutral-500 col-span-full text-center py-10 bg-white border border-neutral-200 rounded-xl shadow-sm">
                No discounted products available matching your selected filters.
              </p>
            )}
